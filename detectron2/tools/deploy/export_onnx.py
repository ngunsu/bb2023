...
# experimental. API not yet final
def export_tracing(torch_model, inputs, half=False):
    assert TORCH_VERSION >= (1, 8)
...
if __name__ == "__main__":
    ...
    eval_data_path = '/datasets/anabalon_osses_no_aug_v2_COCO_format/test/'
    register_coco_instances("my_dataset_val", {}, os.path.join(eval_data_path, "annotations.json"), os.path.join(eval_data_path, "images"))
    ...
    if args.export_method == "caffe2_tracing":
        ...
        sample_inputs = get_sample_inputs(args)
        exported_model = export_tracing(torch_model, sample_inputs, args.half)
    ...
    if args.run_eval:
        ...
        evaluator = COCOEvaluator(dataset, output_dir=args.output, max_dets_per_image=300)
