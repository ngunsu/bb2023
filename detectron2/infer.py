...
def main(args):
    ...
    labels = ['inmaduro', 'mmaduro', 'maduro']
    ...
    for batch, images, scales in batcher.get_batch():
        ...
        for i in range(len(images)):
            ...
            fullname = os.path.join(args.input, os.path.basename(images[i]))
            print("----------- ",n_images, basename, " -----------")
            im = cv2.imread(fullname, 1)
            # Image Visualizations
            output_path = os.path.join(output_dir, "{}.jpg".format(basename))
            visualize_detections(images[i], output_path, detections[i], labels, args.iou_threshold)
            # Text Results
            output_results = ""
            for d in detections[i]:
                f_pred_box = [d['xmin'].item(), d['ymin'].item(), d['xmax'].item(), d['ymax'].item()]
                line = [d['xmin'], d['ymin'], d['xmax'], d['ymax'], d['score'], d['class']]
                output_results += "\t".join([str(f) for f in line]) + "\n"
                print(f_pred_box, d['score'].item(), d['class'])
                ...
                #cv2.rectangle(im, (int(f_pred_box[0]), int(f_pred_box[1])), (int(f_pred_box[2]), int(f_pred_box[3])), colors[d['class']], 3)
            #with open(os.path.join(args.output, "{}.txt".format(basename)), "w") as f:
            #    f.write(output_results)
            #cv2.imwrite(os.path.join(args.output, f'{basename}_.jpg'), im)
            
            n_images += 1

    with open(os.path.join(output_dir, 'pd_coco_infer.json'), 'w') as f:
